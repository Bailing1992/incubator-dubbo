 ...
 * ConditionRouter
 *
 * 条件路由规则 由 两个条件 组成，分别用于对 服务消费者 和 提供者 进行匹配。比如有这样一条规则：
 *
...
public class ConditionRouter extends AbstractRouter {
    ...
    /**
     * 条件路由规则是一条字符串，对于 Dubbo 来说，
     * 它并不能直接理解字符串的意思，需要将其解析成内部格式才行。
    ...
    public void init(String rule) {
        try {
             ...
             * 然后调用 parseRule 方法分别对 服务提供者和消费者规则 进行解析，
             * 最后将 解析结果 赋值给 whenCondition 和 thenCondition 成员变量
             * */
            ...
            // 将 解析出 的 匹配规则 分别赋值 给 whenCondition 和 thenCondition 成员变量
    ...
    public <T> List<Invoker<T>> route(List<Invoker<T>> invokers, URL url, Invocation invocation)
        ...
        try {
            // 先对 服务消费者条件 进行 匹配，如果 匹配 失败，表明 服务消费者 url 不符合 匹配规则，
            // 无需进行后续 匹配，直接返回 Invoker 列表即可。比如下面的规则：
            //     host = 10.20.153.10 => host = 10.0.0.10
            // 这条路由规则 希望 IP 为 10.20.153.10 的 服务消费者 调用 IP 为 10.0.0.10 机器上的服务。
...
//        return true;
    ...
    /**
     * MatchPair 内部 包含了 两个 Set 类型的 成员变量，分别用于 存放匹配 和 不匹配的条件
     * */
